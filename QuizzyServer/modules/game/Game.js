...
exports.Game = class {
    ...
    updateScore(correct, userName) {
        this.logger.debug(`Updating player: ${userName} score for ${correct}`);
        if (correct) {
    ...
    getNextQuestion(userName) {
        this.logger.debug(`Sending next question to player ${userName}`);
        // update user status 
        var questionNumber = this.players[userName].questionNumber++;
        var nextQuestion = this.getQuestion(questionNumber);

        if (nextQuestion) {
            this.players[userName].status = "On question " + (questionNumber + 1);
        }
        else {
            this.players[userName].status = "Finished";
        }
